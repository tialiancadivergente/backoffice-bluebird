...
export function EditSeasonDialog({ season, open, onOpenChange }: Props) {
  ...
  useEffect(() => {
    if (season) {
      form.reset({ name: season.name, active: season.active, launch_id: season.launch_id });
    }
  }, [season, form]);

  const mutation = useMutation({
    mutationFn: (values: FormValues) => updateSeason(season!.id, values),
    onSuccess: () => {
      toast.success("Season atualizada com sucesso!");
      queryClient.invalidateQueries({ queryKey: ["seasons"] });
      onOpenChange(false);
    },
    onError: () => {
      toast.error("Erro ao atualizar a season.");
    },
  });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[420px]">
        <DialogHeader>
          <DialogTitle>Editar Season</DialogTitle>
        </DialogHeader>
